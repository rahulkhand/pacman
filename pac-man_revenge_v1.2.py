...
def chopColors(image):
    (w, h) = image.size
    upper = image.crop((20, 20, w - 20, h/2))
    lower = image.crop((20, h/2 + 20, w - 20, h))
    return(upper, lower)

def split(image):
    top = image.crop((0, 0, image.width, image.height/2 - 20))
    bottom = image.crop((0, image.height/2, image.width, image.height))
    topL = top.crop((3, 0, top.width/2 - 17, top.height))
    topR = top.crop((17 + top.width/2, 0, top.width - 3, top.height))
    bottomL = bottom.crop((3, 10, bottom.width/2 - 17, bottom.height - 10))
    bottomR = bottom.crop((17 + bottom.width/2, 10, bottom.width - 3, bottom.height - 10))
    return [topL, topR, bottomL, bottomR]

# is the pixel a background color
def isBack(r, g, b):
    if (r >= 200) and (g >= 200) and (b >= 200): return True
    else: return False

def notBack(pixel):
    (r, g, b) = pixel
    return not isBack(r, g, b)

# is the pixel a black pixel
def isBlack(r, g, b):
    if ((r <= 50) and (g <= 50) and (b <= 50)): return True
    else: return False
...
def isBlueEye(r, g, b):
    if (r < 20) and (g > 20) and (g < 100) and (b > 230): return True
    else: return False

def isEye(pixel):
    (r, g, b, a) = pixel
    return isOpaque(pixel) and \
        (isBack(r, g, b) or isBlueEye(r, g, b))

def filterImage(image, p):
    (width, height) = image.size
    base = Image.new('RGBA', (width, height))
    template = ImageDraw.Draw(base)
    for y in range(height):
        for x in range(width):
            pixel = image.getpixel((x, y))
            r, g, b = pixel[0], pixel[1], pixel[2]
            if p(pixel): template.point((x, y), fill=(r, g, b, 255))
    return base

def realGhost(image):
    (width, height) = image.size
    transparent = Image.new('RGBA', (width, height))
    template = ImageDraw.Draw(transparent)
    for y in range(height):
        lWall, rWall, trigger = False, False, False
        for x in range(width):
            (r, g, b) = image.getpixel((x, y))
            if (not lWall and isBlack(r,g,b)):  lWall = True
            elif (lWall and (not rWall) and (not isBlack(r, g, b))):
                trigger = True
            elif (lWall and trigger and isBlack(r, g, b)):  rWall = True

            if ((y > 20) and (y < (height - 20))):
                if ((lWall != rWall) or (not isBack(r,g,b))):
                    template.point((x, y), fill=(r,g,b,255))
            elif (not isBack(r, g, b)):
                template.point([(x, y)], (r,g,b,255))
    return transparent
...
def developGhosts(app):
    ...
    blueOrange = ghosts.crop(righthalf)
    (reds, pinks) = chopColors(redPink)
    (blues, oranges) = chopColors(blueOrange)
    allImages = list(map(split, (reds, blues, pinks, oranges)))
    sLen = int(app.cellSize)
    trueGhost = lambda image: realGhost(image).resize((sLen, sLen))
    app.ghostSprites = list(map(lambda x: list(map(trueGhost, x)), allImages))
...
def chomping(app):
    ...
    elif app.mouth == app.maxStretch:   app.widening = True # mouth is shut
    app.ang = 360 - (2  * app.maxStretch) + (2 * app.mouth)
...
def drawPac(app, canvas):
    cx, cy = app.pac.x, app.pac.y
    r = 0.9 * app.cellSize / 2
    ang = app.ang

    if (ang == 360):
        canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill=app.pac.color, 
                            outline=app.pac.color)
        return

    if app.pac.cDir[0] == -1: st = 180 + app.maxStretch
    elif app.pac.cDir[1] == -1: st = 90 + app.maxStretch
    elif app.pac.cDir[1] == 1: st = 270 + app.maxStretch
    else:   st = app.maxStretch
    st -= app.mouth
    canvas.create_arc(cx-r, cy-r, cx+r, cy+r, start=st, extent=ang, 
                        fill=app.pac.color, outline=app.pac.color)

def drawEntities(app, canvas):
    i = 0
    for bod in app.playerList[1:]:
        cx, cy = bod.x, bod.y
        r = 0.9 * app.cellSize / 2
        #canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill=bod.color)
        if bod.color == 'white': selections = app.eyesOnly
        elif bod.color == 'purple': selections = app.scaredyGhosts
        else: selections = app.ghostSprites[i]

        if bod.cDir == None: poster = selections[0]
        elif bod.cDir[0] == -1: poster = selections[3]
        elif bod.cDir[0] == 1: poster = selections[0]
        elif bod.cDir[1] == -1: poster = selections[1]
        else: poster = selections[2]
        canvas.create_image(bod.x, bod.y, image=ImageTk.PhotoImage(poster))
        i += 1
    drawPac(app, canvas)
